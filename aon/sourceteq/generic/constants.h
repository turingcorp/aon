#define colorblue               [UIColor colorWithRed:0.1372 green:0.6705 blue:0.8705 alpha:1]
#define colorred                [UIColor colorWithRed:1 green:0.15 blue:0 alpha:1]
#define celid                   @"cel"
#define fontboldname            @"Arial-BoldMT"
#define fontname                @"ArialMT"
#define headerid                @"header"
#define hubheight               50
#define notclosealert           @"closealert"
#define notpurchaseupd          @"purchaseupd"
#define proppurdouble           @"purchase_double"
#define propmaxscore            @"maxscore"
#define stringdrawing           NSStringDrawingUsesFontLeading|NSStringDrawingUsesLineFragmentOrigin
